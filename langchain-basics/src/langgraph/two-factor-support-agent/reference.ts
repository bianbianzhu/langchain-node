...
Instead, you should use the data sources available to resolve the user's problem in a satisfactory way.`;
    ...
    const requestAuthorization = async (state: AgentState) => {
      ...
      return {
        ...
        authorization_failure_count: hadPreviousAttempt
          ? (state.authorization_failure_count ?? 0) + 1
          : undefined,
      };
    };
    ...
    const confirmAuthorization = async (state: AgentState) => {
      return {
        auth_state:
          state.generated_two_factor_code === state.provided_two_factor_code
            ? "authed"
            : "authorizing",
        provided_two_factor_code: undefined,
    ...
    const executeReadonlyTools = async (state: AgentState) => {
      ...
      // For simplicity, handle single tool calls only
      const toolCall = lastMessage.tool_calls![0];
      const toolResponse = await READONLY_TOOLS_BY_NAME[toolCall.name].invoke(
        toolCall
      );
    ...
    const executeAuthorizedTools = async (state: AgentState) => {
      ...
      // For simplicity, handle single tool calls only
      const toolCall = lastMessage.tool_calls![0];
      const toolResponse = await AUTHORIZED_TOOLS_BY_NAME[toolCall.name].invoke(
        toolCall
      );
      return {
    ...
    if (endState.auth_state === "authorizing") {
      return new Response(
        [
          ...
          `You've had ${
            endState.authorization_failure_count ?? 0
          } failed attempts.`,
        ].join("\n\n")
      );
    } else {
      return new Response(
        endState.messages[endState.messages.length - 1].content
      );
    }
  },
};
